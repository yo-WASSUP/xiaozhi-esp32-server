...
function scopeMultipartRequests(patientId) {
  if (window.__hospiceFetchScoped) return;
...
function syncPatient(user) {
  const patientId = user?.patient_ids?.[0] || '';
  if (!patientId) throw new Error('该患者账号尚未关联患者设备，请联系管理员');
  const changed = localStorage.getItem('hospice_device_id') !== patientId;
  localStorage.setItem('hospice_device_id', patientId);
  localStorage.setItem('xz_tester_deviceMac', patientId);
  localStorage.setItem('hospice_patient_name', user.display_name || user.username);
  return changed;
}
...
export default function AuthGate({ children }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (payload.user?.role !== 'patient') throw new Error('请使用患者账号登录患者端');
        if (syncPatient(payload.user)) window.location.reload();
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      ...
      try {
        ...
        const payload = await response.json();
        if (!response.ok || payload.user?.role !== 'patient' || payload.user?.username !== state.user.username) {
  ...
  const login = async (event) => {
    ...
    try {
      ...
      if (payload.user?.role !== 'patient') {
        ...
        throw new Error('请使用患者账号登录患者端');
      }
      syncPatient(payload.user);
  ...
  const logout = async () => {
    ...
    localStorage.removeItem('xz_tester_deviceMac');
    localStorage.removeItem('hospice_patient_name');
    window.location.reload();
  };
