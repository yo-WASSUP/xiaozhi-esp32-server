...
import { CaretRight, ChatCircleText, VideoCamera } from '@phosphor-icons/react';
    ...
    <div className="screen" style={{ padding: '0 16px 24px' }}>
      ...
      <Card style={{ marginTop: 20, padding: '18px 18px 14px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => setTab('history')} style={{ fontSize: 12, color: C.mist, fontFamily: 'Noto Sans SC', fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 3 }}>查看详情 <CaretRight size={13} /></button>
        </div>
      </Card>
      ...
      <Card style={{ marginTop: 12, padding: '18px' }}>
        ...
        {today && today.conversation_count > 0 ? (
                ...
                {Object.entries(dist).sort((a, b) => b[1] - a[1]).map(([m, n]) => {
                  ...
                  return (
                    <div key={m} style={{ padding: '4px 12px', borderRadius: 12, background: `${meta.color}18`, border: `1px solid ${meta.color}33`, fontSize: 12, color: meta.color, fontFamily: 'Noto Sans SC', fontWeight: 300 }}>
                      <span style={{ display: 'inline-flex', verticalAlign: 'middle', marginRight: 4 }}><MoodIcon mood={m} size={15} decorative /></span>
                      {meta.label}，{n} 次
                    </div>
                  );
                })}
          ...
          <div style={{ fontSize: 14, color: C.inkFaint, fontWeight: 400, lineHeight: 1.8, textAlign: 'center', padding: '12px 0' }}>
