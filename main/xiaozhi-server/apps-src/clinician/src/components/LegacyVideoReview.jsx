...
const C = {
  ink: '#24362e', inkMid: '#455b50', inkFaint: '#68776f', mist: '#bdcbc3',
  sage: '#39745a', amber: '#c38b45', red: '#a94742', green: '#4f806a',
};

export default function LegacyVideoReview({ deviceId }) {
  ...
  const loadVideoSource = async () => {
    try {
      const r = await fetch(`/api/hospice/video/source?device_id=${encodeURIComponent(deviceId)}`);
  ...
  const saveAssetLabel = async (asset) => {
    try {
      const r = await fetch('/api/hospice/video/assets', {
        ...
        body: JSON.stringify({ device_id: deviceId, url: asset.url, label: asset.label }),
      });
      const j = await r.json();
  ...
  const deleteAsset = async (asset) => {
    try {
      const r = await fetch('/api/hospice/video/assets', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ device_id: deviceId, url: asset.url }),
      });
      const j = await r.json();
...
const toastStyle = { position: 'absolute', bottom: 22, left: '50%', transform: 'translateX(-50%)', maxWidth: 'calc(100% - 40px)', background: C.ink, color: '#fff', borderRadius: 14, padding: '8px 14px', fontSize: 13, zIndex: 100, fontFamily: 'Noto Sans SC' };
