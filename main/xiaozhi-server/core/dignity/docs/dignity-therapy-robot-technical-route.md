...
- 主动发起话题，而不是只等待患者提问。
- 按阶段推进人生回顾、价值提炼、重要关系、留言祝福等治疗目标。
- 根据患者情绪、回答质量和访谈进度动态调整策略。
- 形成可追踪、可编辑、可复盘的访谈路径。
- 输出生命故事文稿、家属照片线索和科研分析数据。
- 通过眼睛表情、耳朵动作、手部动作、香薰等外设形成陪伴感。

边界上，AI 不替代心理医生、护士或临床判断。系统用于访谈陪伴、资料整理、初步风险提示和内容生成，关键内容应保留护士或研究人员确认环节。
...
首版建议以 Python 侧新增独立模块接入，而不是大改现有服务：
...
为保证 LLM 输出可控，在 ASR 后、LLM 生成前、LLM 生成后增加控制模块。
...
首版不建议重度依赖 function calling 做意图识别。可以优先采用轻量分类、规则判断和结构化 JSON 输出结合的方式。
...
- 节点策略表。
- 10 组测试用例。
- 本地命令行 Demo：输入患者文本，输出阶段、策略、回复和动作枚举。
...
以下内容可以作为内容侧任务的 v1 初稿，后续由项目成员补充真实调研、护士意见和华西科研反馈。
...
- 确定 LangGraph 概念验证 的技术边界和接入位置。
- 验证安卓平板通过 USB/蓝牙向外部模块发送控制信号的能力。
- 判断是否需要更换工业安卓平板。
- 调研小智生态是否支持大尺寸眼睛硬件和 emoji 情绪模块复用。
- 调研适配 8-9 cm 宽、10-12 cm 高的大尺寸眼睛模组。
- 梳理 AI 对话状态机、节点策略和外设动作之间的接口。
